...
class Tokenizer:
    ...
    def next(self):
        ...
        while True:
            if len(self._line) == 0:
                ...
                if (self._ofile != None):
                    self._ofile.writeXML('source', '/// %d: %s' %
                                         (self._lineNum, self._rawline))
    ...
    def _parseToken(self):
        ...
        while len(self._line):
            ...
            raise JackError('Sytax error in line ' +
                            str(self._lineNum) + ': illegal character "' + c + '".')
    ...
    def _parseInt(self):
        ...
        while len(self._line):
            ...
            if c in numberChars:
                ...
                if c in ';)] ':
                    break
                raise JackError('Syntax error in line ' +
                                str(self._lineNum) + ': invalid integer.')
            if (ret > 32767):  # Zadatak 1b
                raise JackError('Integer overflow in line ' +
                                str(self._lineNum) + ': constant must be less than 32768.')
            self._line = self._line[1:]
        return ret
    ...
    def _parseString(self):
        ...
        while len(self._line):
            ...
            if c == '"':
                ...
                self._line = self._line[1:]
        raise JackError('Sytax error in line ' +
                        str(self._lineNum) + ': open string constant.')
